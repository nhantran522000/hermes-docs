...
Full lifecycle, event bridge, and approval flow: [ACP Internals](acp-internals.md).
...
Setup, headers (`X-Hermes-Session-Id`, `X-Hermes-Session-Key`), and frontend wiring: [API Server](../user-guide/features/api-server.md).
...
- **You want a Python in-process embed without a subprocess** → import `run_agent.AIAgent` directly. See [Agent Loop](agent-loop.md).
