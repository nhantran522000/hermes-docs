...
- [Bitwarden Secrets Manager](bitwarden.md) — `bws` CLI, lazy-installed, free tier works.
- [1Password](onepassword.md) — `op://` references via the official `op` CLI; service-account or desktop session auth.
- [Command helper](https://hermes-agent.nousresearch.com/docs/user-guide/secrets/command) — any CLI vault (`keepassxc-cli`, `secret-tool`, `pass`, custom scripts) via a user-configured helper that prints `KEY=VALUE` lines.
...
Two orchestrator-level knobs make one shared vault safe across [profiles](https://hermes-agent.nousresearch.com/docs/user-guide/features/profiles):
