...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`hermes send` CLI](https://hermes-agent.nousresearch.com/docs/guides/pipe-script-output):
