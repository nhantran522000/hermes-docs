...
These env vars are NOT set on the host — they're injected into Docker sandboxes by the [Egress proxy](https://hermes-agent.nousresearch.com/docs/user-guide/egress/iron-proxy) integration when `proxy.enabled: true`. Docker is the only wired backend in this release.
...
| `NODE_OPTIONS` | Appended with `--use-openssl-ca` (your existing flags are preserved) so Node.js routes through the OpenSSL store the other CA-bundle vars control. Narrows the [Node.js asymmetric CA caveat](https://hermes-agent.nousresearch.com/docs/user-guide/egress/iron-proxy#nodejs-asymmetric-ca-caveat). |
...
These are set automatically by the Docker terminal backend when `proxy.enabled: true` AND the daemon is running. You don't set them yourself; the relevant operator-facing knobs are in `~/.hermes/config.yaml` under the `proxy:` section — see [Egress proxy → Configuration](https://hermes-agent.nousresearch.com/docs/user-guide/egress/iron-proxy#configuration).
...
| `PHOTON_TELEMETRY` | Enable Spectrum SDK telemetry in the sidecar (`true`/`false`, default `false`; toggle with \`hermes photon telemetry on |
